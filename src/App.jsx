...
function UserMessage({ text, time }) {
  return (
    <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-3xl px-5 py-3.5 shadow-md">
      ...
      <p className="text-xs text-blue-200 mt-2 text-right">{time}</p>
    </div>
  );
}

function AIMessage({ text, hasData, dataSource, dataContent }) {
  return (
...
function EmptyRadiologistState() {
  return (
    <div className="text-center py-20">
...
function NotificationCard({ notification }) {
  ...
  const IconComponent = isUrgent ? AlertCircle : Clock;

  return (
    <div
      className={`mb-4 border-l-4 ${borderColor} ${bgColor} rounded-r-3xl shadow-sm animate-pulse`}
      ...
      <div className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <p className={`font-bold text-sm mb-2 ${textColor}`}>
              {isUrgent ? (
            ...
            <p className="text-sm text-slate-700 leading-relaxed mb-3">{notification.message}</p>
...
function App() {
  ...
  async function processMessage(input) {
    ...
    setMessages(prev => [...prev, { sender: 'ai', text: '', time: formatTime(), hasData: false }]);
    ...
    if (lowerInput.includes('acr') || lowerInput.includes('criteria') || lowerInput.includes('appropriateness')) {
      updateLastMessage({
        hasData: true,
  ...
  async function handleSendMessage() {
    ...
    setMessages(prev => [...prev, { sender: 'user', text: input, time: formatTime() }]);
    setUserInput('');
  ...
  async function runDemo() {
    ...
    for (const step of DEMO_SEQUENCE) {
      ...
      setMessages(prev => [...prev, { sender: 'user', text: step.input, time: formatTime() }]);
      setUserInput('');
  ...
  const isInputDisabled = isTyping || isRunningDemo;
  const isSendDisabled = isTyping || !userInput.trim() || isRunningDemo;
        ...
        <div className="flex-1 flex gap-0 overflow-hidden shadow-2xl rounded-3xl">
          <div className="flex-1 flex flex-col bg-slate-50 rounded-l-3xl">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {messages.length === 0 && <EmptyClinicianState />}
          ...
          <div className="flex-1 flex flex-col bg-slate-800 rounded-r-3xl border-l border-slate-700">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {notifications.length === 0 && <EmptyRadiologistState />}
